...
## Status
Accepted (Updated — domain extraction step removed; code generation retained; multi-pass recovery added — see ADR-012)
...
**Update (ADR-012):** A token budget applied in `generate_service_layer` previously dropped low-priority methods silently when the method list was too large to fit in one prompt. This is now addressed by a multi-pass recovery loop (`MultiPassMerger`) that runs additional LLM calls for the dropped remainder and merges all passes into a single output file. See [ADR-012](ADR-012-multi-pass-token-recovery.md) for the full decision record.
