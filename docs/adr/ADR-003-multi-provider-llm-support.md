...
- `LLMClient` exposes a `_last_call_was_truncated: bool` flag that is reset to `False` at the start of every `generate()` call and set to `True` if `_truncate_prompt()` fires. Generators read this flag immediately after `generate()` returns to decide whether to trigger multi-pass recovery (see ADR-012).
...
- The `_last_call_was_truncated` flag is not thread-safe. This is acceptable because each `LLMCodeGenerator` owns its own `LLMClient` instance; a shared `LLMClient` across threads would require per-call state instead.
