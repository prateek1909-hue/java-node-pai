...
from src.generators.multi_pass_merger import MultiPassMerger
...
[
  {{
    ...
    def generate_service_layer(
        ...
        # Keep the full list so sub-case B (truncation without a clean dropped list)
        # can compare against it in the recovery pass.
        all_original_methods = list(methods_info)
        ...
        methods_info, dropped_methods = budget_methods(
            methods_info, METHODS_TOKEN_BUDGET
        )
        if dropped_methods:
            logger.warning(
                "generate_service_layer(%s): methods_info reduced from %d → %d methods "
                "(%d dropped — accessors and low-complexity methods first). "
                "Multi-pass will recover dropped methods if ENABLE_MULTI_PASS=true.",
                service_name,
...
{self._get_code_instruction()}"""
        ...
        first_pass_code = self._extract_code_from_response(response)

        # --- multi-pass recovery ---
        if self.settings.enable_multi_pass:
            prompt_was_truncated = self.llm_client._last_call_was_truncated
            ...
            if merger.needs_extra_pass(dropped_methods, prompt_was_truncated):
                logger.info(
                    "generate_service_layer(%s): starting multi-pass "
                    "(dropped=%d, truncated=%s, max_passes=%d).",
                    service_name,
                    len(dropped_methods),
                ...
                return merger.run(
                    ...
                    all_original_methods=all_original_methods,
